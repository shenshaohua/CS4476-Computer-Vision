...
def make_dataset(path: str) -> Tuple[List[str],List[str]]:
  ...
  for name in os.listdir(path):
      if name[1] == "a":
        images_a.append(os.path.join(path, name))
      else:
        images_b.append(os.path.join(path, name))
  images_a.sort()
  images_b.sort()
...
def get_cutoff_frequencies(path: str) -> List[int]:
  ...
  ############################
  ### TODO: YOUR CODE HERE ###
  cutoff_frequencies = np.array([])
  fo = open(path, "r")
  cfs = fo.readlines()
  cutoff_frequencies = []
  for n in cfs:
    cutoff_frequencies = np.append(cutoff_frequencies, int(n))
  fo.close()
...
class HybridImageDataset(data.Dataset):
  ...
  def __getitem__(self, idx: int) -> Tuple[torch.Tensor,torch.Tensor,int]:
    ...
    apath = self.images_a[idx]
    bpath = self.images_b[idx]
    imga = PIL.Image.open(apath)
    imgb = PIL.Image.open(bpath)
