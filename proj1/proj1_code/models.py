...
class HybridImageModel(nn.Module):
  ...
  def get_kernel(self, cutoff_frequency: int) -> torch.Tensor:
    ...
    k = gauss.shape[0]
    gauss = np.tile(gauss, (c, 1))
    gauss = np.reshape(gauss, (c, 1, k, k))
  ...
  def low_pass(self, x, kernel):
    ...
    filtered_image = F.conv2d(x, kernel.float(), padding = pad, groups = self.n_channels)
  ...
  def forward(self, image1, image2, cutoff_frequency):
    ...
    filtered = self.low_pass(image2, kernel)
    low_frequencies = self.low_pass(image1, kernel)
    high_frequencies = image2 - filtered
